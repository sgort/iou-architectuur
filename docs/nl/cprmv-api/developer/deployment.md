...
The CPRMV API runs as a Docker container. The image is built and pushed by
GitLab CI to Docker Hub (`datafluisteraar/cprmv-api`).
...
- Exposes port `8000`.
- Health check via `urllib.request.urlopen('http://localhost:8000/',
  timeout=5)`.
...
The `data/` volume mount allows updating the XSLT files and method TTLs without
...
The original deployment target documented in `serve_api/README.md` is a Synology
NAS:
...
The `IMAGE_TAG` environment variable can pin to a specific commit SHA from the
CI build output.
...
The container's built-in health check polls `http://localhost:8000/` every 30
seconds with a 30-second timeout, 3 retries, and a 5-second start period. The
`/` endpoint returns `{"CPRMV Rules Serve API": "0.4.0"}`.
