...
The server starts on `http://127.0.0.1:8000`. The Swagger UI is at
`http://127.0.0.1:8000/docs`.

!!! note "Working directory"
    `serve.py` uses relative paths to load
    `./data/cprmvmethods.ttl`, `./data/cprmv.ttl`, and `./respec/`. The server must
    be started from `serve_api/`.
...
These files are included in the repository and copied into the Docker image at
build time.
