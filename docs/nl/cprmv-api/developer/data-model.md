...
The CPRMV API produces and consumes data conforming to the CPRMV OWL vocabulary
(`rdf/cprmv.ttl`) validated by the SHACL shapes in `rdf/cprmv.shacl.ttl`.
...
A set of rules that is the output of a public service. Subclass of
`eli:LegalResource` and `cv:Output`.
...
Subclass of `cprmv:RuleSet`. A rule set produced by following an analysis method
(e.g. Wetsanalyse/JAS).

### cprmv:DecisionModel

Subclass of `cprmv:RuleSet`. A rule set produced by following a formalisation
method (e.g. DMN 1.3).
...
Subclass of `cprmv:Rule`. A rule that represents a parameter value (e.g. income
...
`cprmv:hasPart` uses standard RDF lists (`rdf:first` / `rdf:rest` / `rdf:nil`).
The Python traversal in `get_rule` and
`get_first_rule_by_Id_path_split_from_rule_as_root` walks these lists
explicitly. Each list item is a blank node `[ a cprmv:Rule; cprmv:id "..."; ...
]`.
...
`rdf/cprmv.shacl.ttl` defines shapes for `cprmv:RuleSet`, `cprmv:DecisionModel`,
`cprmv:Rule`, and `cprmv:RuleMethod`. The shapes enforce mandatory properties
and cardinality constraints as listed in the tables above. They can be used to
validate TTL files produced by the CLI tool (`tools/bwb2cprmv/`) or the API's
own output before ingestion into TriplyDB.
