...
The site is built on MkDocs with the Material theme, the mkdocs-static-i18n
plugin for dual-language support, and the git-revision-date-localized plugin for
showing last-modified timestamps.
...
deploys to Azure Static Web Apps. The `acc` branch deploys to the acceptance
environment.
