...
The frontend is a React 19 TypeScript SPA built with Vite. It has no routing
library — navigation is a local state enum. It has no global state management
library — state lives in component hooks, with `localStorage` as the only
persistence layer.
...
The Chain Composer uses `@dnd-kit/core` for drag detection and
`@dnd-kit/sortable` for reordering within the composer. DMN cards in the
Available DMNs list are `Draggable`; the Chain Composer area is a `Droppable`.
Cards already in the composer use `SortableContext` for reordering.

---

## SPARQL service

`sparqlService.ts` handles query execution via the backend proxy endpoint
(`/v1/triplydb/query`). For direct SPARQL endpoints in development, it includes
a CORS fallback. Result parsing handles both standard
`application/sparql-results+json` and variations in binding formats.

---

## Template service (localStorage)

`templateService.ts` provides the localStorage interface for both chain
templates and BPMN processes. All storage keys are prefixed with
`linkeddata-explorer-`. Templates are namespaced by endpoint URL so switching
endpoints does not surface templates from another dataset.
...
Build targets: `npm run build` (production), `npm run build:acc` (acceptance),
