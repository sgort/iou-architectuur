...
This page covers end-to-end test procedures for the validation system, semantic
chain detection, DRD compatibility checks, and template management. Use this as
a QA checklist before pushing to ACC.
...
- Active TriplyDB endpoint:
  `https://api.open-regels.triply.cc/datasets/stevengort/DMN-discovery/services/DMN-discovery/sparql`
- `ZorgtoeslagVoorwaardenCheck.ttl` uploaded to the test TriplyDB dataset
  (required for semantic chain tests)
...
- Six semantic match log entries for: `isIngezetene`, `heeftJuisteLeeftijd`,
  `betalingsregelingOK`, `vrijVanDetentie`, `heeftGeldigeVerzekering`,
  `inkomenBinnenGrenzen`
- Validation panel: amber warning, "Sequential execution required (semantic
  links)"
...
4. Switch to a different endpoint: verify neither template appears
   (endpoint-scoped)
...
7. Select it: verify `camunda:decisionRef` populates with the `dmn1_` prefixed
   identifier
...
- Check TriplyDB for `dct:subject` integrity — must point to variable URI, not
  DMN URI
...
- Verify Operaton is accessible: `curl
  https://operaton.open-regels.nl/engine-rest/version`
...
- If `semantic > 0` despite visual identifier match, check for variable type
  mismatch in TriplyDB (`"Boolean"` vs `"boolean"`)
