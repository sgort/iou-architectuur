...
This page covers the SPARQL query, TypeScript interfaces, and UI components that
implement the vendor badge and detail modal.
...
The backend groups multiple vendor records per DMN identifier in
post-processing, building the `vendors` array on each `DmnModel`.
...
`e.stopPropagation()` prevents the card expand/collapse from triggering when the
badge is clicked.
...
The vendor modal renders a section per vendor in `dmn.vendors`. Key rendering
logic for licence and access type badges:
...
Contact fields render as links: `href="mailto:{email}"`, `href="tel:{phone}"`,
`href="{serviceUrl}"` with `target="_blank"`.
...
The vendor vocabulary (`ronl:VendorService`, `ronl:basedOn`,
`ronl:implementedBy`, `ronl:accessType`) is part of the RONL Ontology v1.0. For
the full specification, see [CPSV Editor — RONL
