...
RONL Business API validates every request against a JWT access token issued by
Keycloak. The token contains standard OIDC claims plus custom claims injected
...
These claims are added by Keycloak protocol mappers configured on the
`ronl-business-api` client:
...
After successful JWT validation in `jwt.middleware.ts`, the decoded payload is
...
The tenant middleware reads `req.user.municipality` to load the `TenantConfig`
and apply the feature allowlist for the request.
...
The Keycloak JS adapter in the frontend automatically refreshes the access token
before it expires, as long as the SSO session is still valid.
