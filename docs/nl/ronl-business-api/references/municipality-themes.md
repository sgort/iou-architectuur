...
Each municipality has a colour theme defined in
`packages/frontend/public/tenants.json`. The theme is applied at login time
based on the `municipality` JWT claim.
...
Tailwind utility classes in the frontend use `var(--color-primary)` etc. as
their colour values. This means the entire theme switches dynamically — no page
reload, no per-municipality CSS bundle.

---

## Adding a theme

See [Adding a Municipality](../user-guide/adding-municipality.md) for the
