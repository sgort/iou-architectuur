...
**Problem:** Browser CORS policy blocks direct calls from `localhost:5173` to
`brp-api-mock.open-regels.nl`
...
The timeline doesn't store historical snapshots. Instead, it **derives**
...
In production, BSN comes from DigiD via Keycloak. For testing, we map usernames
to BSNs.
...
2. **User Context** - BSN derived from authenticated user (no arbitrary BSN
   queries)
...
1. **LoA Requirement** - Timeline requires DigiD LoA "hoog" (substantial
   assurance)
2. **BSN in Token** - DigiD provides BSN via SAML → Keycloak → JWT
3. **Session Management** - Keycloak handles DigiD session timeout
4. **Mandate Support** - (Future) Representatives can view citizen timelines
   with proper mandate
...
**Why:** Project references caused type-checking errors. The path alias
`"@ronl/shared": ["../shared/src"]` was removed, and imports now resolve via npm
workspaces to the compiled `dist` folder.
...
**Why:** The `composite: true` setting was preventing proper `.d.ts` file
generation.
...
**Why:** The new `brp.api.ts` file also contains `localhost:3002` URLs that need
to be replaced.
