...
RONL Business API uses four GitHub Actions workflows for continuous integration
and deployment. All workflows run on `ubuntu-latest`.
...
Workflows only trigger when relevant files change, avoiding unnecessary
deployments:

- Backend workflows: `packages/backend/**`, `packages/shared/**`, or the
...
Changes are developed on feature branches, merged to `acc` for acceptance
testing, and promoted to `main` for production. The backend PROD deployment has
a mandatory manual approval gate to prevent accidental production changes.
...
To rotate a publish profile: Azure Portal → App Service → Overview → Get publish
profile → download, then update the GitHub secret with the file contents.
...
Or use the Azure Portal: App Service → Deployment Center → select a previous
deployment → Redeploy.

**Frontend** — Azure Static Web Apps keeps deployment history. In the Azure
Portal: Static Web App → Environments → select a previous deployment → Promote.
