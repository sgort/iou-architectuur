...
The backend deploys to **Azure App Service** (Node.js 20) via GitHub Actions.
There are two independent deployment targets: ACC and PROD. PROD requires a
...
!!! warning "Deployment package structure"
    The `dist/` folder must be copied as
    a folder (`cp -r dist deploy/`), not flattened (`cp -r dist/* deploy/`).
    Flattening breaks TypeScript module resolution paths and causes 404 errors on
    all `/v1/*` endpoints after deployment.
...
Azure App Settings (environment variables) are configured via CLI or the Azure
Portal. Set the production values from
...
The full variable reference is in [Environment
...
For PROD, substitute `ronl-business-api-acc` → `ronl-business-api-prod`,
`rg-ronl-acc` → `rg-ronl-prod`, and the ACC URLs → PROD URLs.
...
If the health check fails after 5 attempts, the workflow fails and the
deployment is marked unsuccessful. Roll back by redeploying the previous commit
or using an Azure deployment slot swap.
