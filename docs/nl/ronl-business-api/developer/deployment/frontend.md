...
The frontend is deployed to Azure Static Web Apps via GitHub Actions. Separate
instances run for ACC and PROD environments.
...
**Critical for React Router:** Azure Static Web Apps needs
`staticwebapp.config.json` to handle client-side routing.
...
**Without this file:** Routes like `/auth` and `/dashboard` return 404 errors
after Keycloak redirect.
