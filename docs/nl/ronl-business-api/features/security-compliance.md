...
RONL Business API is designed to meet Dutch government security requirements.
The following standards inform the security architecture and audit requirements.
...
All traffic between components is encrypted. Caddy (the VM reverse proxy)
handles TLS termination for Keycloak and Operaton using automatically-renewed
Let's Encrypt certificates. Azure manages certificates for the Static Web App
and App Service.
...
- **General API** — 100 requests per 15 minutes per IP (or per tenant+IP if
  `RATE_LIMIT_PER_TENANT=true`)
- **Authentication endpoints** — 5 requests per 15 minutes per IP

Keycloak adds brute-force protection independently: 5 failed login attempts
trigger a 15-minute lockout with exponential backoff.
...
All other origins receive HTTP 403. Keycloak client settings mirror these
origins in `Web Origins` and `Valid Redirect URIs`.
...
Tokens failing any check are rejected with HTTP 401. No request data is
processed before successful validation.

### Secrets management

Secrets are stored as Azure App Settings (environment variables on App Service)
and VM `.env` files (not committed to git). The repository contains only
`.env.example` templates. The following are never in version control:
...
Every API call that results in a process action is written to the PostgreSQL
audit log with:
...
Audit records have a configured retention of 2555 days (7 years) to satisfy BIO
and AVG archiving requirements. The table schema is initialised by
...
- BSN (Citizen Service Number) is encrypted at rest and never logged in
  plaintext
