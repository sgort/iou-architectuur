...
> How a release reaches production: [Deployment](deployment.md#how-a-promotion-reaches-production). The Thuisbatterij processes: [BPMN Modeler](bpmn-modeler.md). Measured suites: [Testing](testing.md).
...
> The integration in prose: [DSO Integration](../features/dso-integration.md#the-six-dso-apis-behind-the-viewer). The published contract: [API Specification](../reference/api-specification.md). What a release is checked against: [Post-deployment verification](deployment.md#post-deployment-verification). Measured suites: [Testing](testing.md).
...
**Smaller fixes.** A disallowed CORS origin gets an ordinary response without an `Access-Control-Allow-Origin` header instead of a 500 ([#145](https://github.com/sgort/linked-data-explorer/issues/145)). DMN XML export fetches `/v1/dmns/{identifier}/xml` with the identifier URL-encoded; a duplicate legacy handler mounted ahead of the router had been answering every `/api/dmns/:x/xml` request, so that alias never sent its deprecation headers — it now does ([#132](https://github.com/sgort/linked-data-explorer/issues/132)). `form_schemas.status` and `document_templates.status` are `NOT NULL`, with any NULL backfilled on start ([#151](https://github.com/sgort/linked-data-explorer/issues/151)). The error banner and Settings panel animations, which named a Tailwind plugin that was never installed, are real now, and Settings closes on the full-width views it would have covered ([#160](https://github.com/sgort/linked-data-explorer/issues/160), [#76](https://github.com/sgort/linked-data-explorer/issues/76)).
...
- **`POST /v1/dmns/evaluate/:decisionKey`** proxies an evaluate call server-side. Both routes exist for the same reason: the CPSV Editor called Operaton directly from the browser, which CORS blocks for a local dev origin. The evaluate proxy forwards Operaton's response **byte-for-byte and status-for-status** — the raw success array or exception object, *not* the usual `{success, data, error}` envelope — because the DMN tab reads Operaton's own JSON. A new `evaluateRaw()` passes variables straight through, skipping the type inference `evaluateDecision()` does for its different caller contract, which would otherwise double-wrap an already Operaton-shaped body. See [API Reference](../reference/api-specification.md) and the CPSV Editor's [DMN Implementation](../../cpsv-editor/developer/dmn-implementation.md#operaton-calls-go-through-the-backend-v2026080).
